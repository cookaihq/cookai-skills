...
| [pdf2markdown](pdf2markdown/) | 把一个 PDF（本地文件或公开 HTTPS 链接）转成可复核的 Markdown 工作目录：preflight 门禁、一次一个 Doc2X 转换任务（创建 + 轮询 + 原子采纳结果 ZIP 为不可变原始产物）、逐处证据绑定的人工复核与修正、原始产物全程保留，中断后可从任意阶段续跑；付费重试必须显式确认 | [SKILL.md](pdf2markdown/SKILL.md) |
