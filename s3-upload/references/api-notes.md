...
Two properties make the file readable by a separate process that never sees this command's exit code:

- **The file always belongs to the current run.** As soon as the preflight passes and before the first remote request, a `not_started` placeholder is written; every later write replaces it. A run that dies without a durable result (for example a `DefinitiveNoWrite` 403) therefore leaves `not_started`, never the previous run's `ok`. If that placeholder cannot be written, the upload stops at exit 1 with `result_error` and issues no remote request.
