...
Conditional Put (`ConditionalPutObject`, backing `collision=unique|reject` via `If-None-Match: *`) is enabled on the `aws-s3` and `cloudflare-r2` presets, with the providers' official conditional-write documentation as evidence. On an asserted `custom` baseline it is `disabled`; on the `aliyun-oss` and `tencent-cos` candidates it is `test-only`, because only `PutObject` and `PresignGetObject` reach `experimental` there. Neither state executes in normal mode — `test-only` needs the maintainer live interlock — so `collision=unique|reject` blocks the plan on all three. Under `reject`, a remote 412 is followed by one presigned full-body GET: a size+SHA-256 double match against the planned source adopts the existing object (`adopted`, exit 0), anything less is a collision (exit 4).
